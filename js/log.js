...
const LOG_ACTION_LABELS = {
  ...
  field_changed: (field, oldVal, newVal) => {
    const labels = {
      ...
      driver3: "Driver 3",
      driver4: "Driver 4",
      driver1Status: "Driver 1 status",
      driver2Status: "Driver 2 status",
      driver3Status: "Driver 3 status",
      driver4Status: "Driver 4 status",
    };
...
async function loadDriversAndBuses(forceRefresh = false) {
  ...
  if (!forceRefresh) {
    ...
    if (cDrivers)
      state.driversList = sortDriversForDisplay(
        cDrivers.map(normalizeDriverRecord).filter((d) => d.driverName),
      );
  ...
  if (freshDrivers) {
    state.driversList = sortDriversForDisplay(
      freshDrivers.map(normalizeDriverRecord).filter((d) => d.driverName),
    );
  }
  ...
  if (state.busesList.length) CACHE.set("cache_buses", state.busesList, CONFIG.CACHE_TTL_BUSES);

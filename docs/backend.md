...
function weekData_(p) {
  ...
  const aDriver4Pay = idx(asnHdr, "driver4Pay");
  ...
  if (asnVals.length > 1 && tripKeysInRange.size) {
    for (let r = 1; r < asnVals.length; r++) {
      ...
      assignments.push({
        ...
        driver1Pay: aDriver1Pay >= 0 ? String(row[aDriver1Pay] || "").trim() : "",
        driver2Pay: aDriver2Pay >= 0 ? String(row[aDriver2Pay] || "").trim() : "",
        driver3Pay: aDriver3Pay >= 0 ? String(row[aDriver3Pay] || "").trim() : "",
        driver4Pay: aDriver4Pay >= 0 ? String(row[aDriver4Pay] || "").trim() : "",
      });
    }
...
function replaceBusAssignments_(busSheet, tripKey, p) {
  ...
  for (let i = 1; i <= busesNeeded; i++) {
    ...
    const driver1Pay = String(p[`bus${i}_driver1Pay`] || "").trim();
    const driver2Pay = String(p[`bus${i}_driver2Pay`] || "").trim();
    const driver3Pay = String(p[`bus${i}_driver3Pay`] || "").trim();
    const driver4Pay = String(p[`bus${i}_driver4Pay`] || "").trim();
    const rowObj = {
      tripKey: tripKey,
      busNumber: i,
      busId: busId,
      ...
      driver1Pay: driver1Pay,
      driver2Pay: driver2Pay,
      driver3Pay: driver3Pay,
      driver4Pay: driver4Pay,
    };
...
function mapTripFromParams_(p, base) {
  ...
  return {
    ...
    drivingHours: String(p.drivingHours || "").trim(),
    onDutyHours: String(p.onDutyHours || "").trim(),
